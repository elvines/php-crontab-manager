 ...
 *             next() and previous().
 *
 * Copyright 2009-2012 Brugman Holding B.V. & Joost Brugman. A separate license is required to modify, republish or otherwise use this file.
 *
 */
class Schedule {
    /**
    ...
    /**
     * Performs only base initialization, including language initialization.
     *
     * @param string $languageThe languagecode of the chosen language.
     */
    ...
    /**
     * Creates a new Schedule object based on a Cron specification.
     *
     * @param string $cronSpec A string containing a cron specification.
     * @param string $language The language to use to create a natural language representation of the string
     * @return \Cron\Schedule A new Schedule object. An \Exception is thrown if the specification is invalid.
     * @throws \Exception
     */
    final public static function fromCronString($cronSpec = '* * * * * *', $language = 'en') {
        ...
        $elementsCount = count($elements = preg_split('/\s+/', $cronSpec));
        if ($elementsCount < 5) {
            throw new \Exception('Invalid specification.');
        }

        // if year is missed - add '*'
        if ($elementsCount == 5) {
            $cronSpec .= ' *';
    ...
    /**
     * Interprets a single field from a cron specification. Throws an \Exception if the specification is in some way invalid.
     *
     * @param string $specification The actual text from the spefication, such as 12-38/3
     * @param int $rangeMin The lowest value for specification.
     * @param int $rangeMax The highest value for specification
     * @param array $namedItems A key/value pair where value is a value between $rangeMin and $rangeMax and key is the name for that value.
     * @param string $errorName The name of the category to use in case of an error.
     * @return array An array with entries, each of which is an array with the following fields:
    ...
    final private function cronInterpret($specification, $rangeMin, $rangeMax, $namedItems, $errorName) {
        ...
        foreach ($arrSegments as $segment) {
            ...
            if (!$hasRange && $hasInterval) {
               //throw new \Exception("Invalid Range ($errorName)."); // it's should be valid for '*/3 * * * * *'
            }
    ...
    /**
     * Uses the interpreted cron specification of a single item from a cron specification to create an array with keys that match the
     *				selected items.
     *
     * @param type $cronInterpreted The interpreted specification
     * @return array An array where each key identifies a matching entry. E.g. the cron specification *\/*10 for minutes will yield an array
     *				[0] => 1
     *				[10] => 1
     *				[20] => 1
     *				[30] => 1
    ...
    /**
     * Transforms a flexible parameter passing of a datetime specification into an internally used array.
     *
     * @param mixed $time If a string interpreted as a datetime string in the YYYY-MM-DD HH:II format and other parameters ignored.
     ...
     * @return array An array with indices 0-4 holding the actual interpreted values for $minute, $hour, $day, $month and $year.
     */
    ...
    /**
     * Returns TRUE if the specified date and time corresponds to a scheduled point in time. FALSE otherwise.
     *
     * @param mixed $time If a string interpreted as a datetime string in the YYYY-MM-DD HH:II format and other parameters ignored.
     *									If an array $minute, $hour, $day, $month and $year are passed as keys 0-4 and other parameters ignored.
     *									If a string, interpreted as unix time.
     *									If omitted or specified FALSE, defaults to the current time.
     * @return boolean TRUE if the schedule matches the specified datetime. FALSE otherwise.
     */
    ...
    /**
     * Acquires the first scheduled datetime beyond the provided one.
     *
     * @param type $time
     * If a string interpreted as a datetime string in the YYYY-MM-DD HH:II format and other parameters ignored.
     *									If an array $minute, $hour, $day, $month and $year are passed as keys 0-4 and other parameters ignored.
     *									If a string, interpreted as unix time.
     *									If omitted or specified FALSE, defaults to the current time.
     * @return int An array with the following keys:
    ...
    /**
     * Advances the current item to the next one (the next minute, the next hour, etc.).
     *
     * @param array $arrItems A reference to the collection in which to advance.
     * @param int $rangeMin The lowest possible value for $current.
     * @param int $rangeMax The highest possible value for $current
     * @param int $current The index that is being incremented.
     * @return boolean FALSE if current did not overflow (reset back to the earliest possible value). TRUE if it did.
     */
    ...
    /**
     * Retrieves the earliest item in a collection, e.g. the earliest minute or the earliest month.
     *
     * @param array $arrItems A reference to the collection in which to search.
     * @param int $afterItem The highest index that is to be skipped.
    ...
    /**
     * Acquires the first scheduled datetime before the provided one.
     *
     * @param type $time
     * If a string interpreted as a datetime string in the YYYY-MM-DD HH:II format and other parameters ignored.
     *									If an array $minute, $hour, $day, $month and $year are passed as keys 0-4 and other parameters ignored.
    ...
    /**
     * Recedes the current item to the previous one (the previous minute, the previous hour, etc.).
     *
     * @param array $arrItems A reference to the collection in which to recede.
     * @param int $rangeMin The lowest possible value for $current.
     * @param int $rangeMax The highest possible value for $current
     * @param mixed $current The index that is being decremented.
     * @return boolean FALSE if current did not overflow (reset back to the highest possible value). TRUE if it did.
     */
    ...
    /**
     * Retrieves the latest item in a collection, e.g. the latest minute or the latest month.
     *
     * @param array $arrItems A reference to the collection in which to search.
     * @param int $beforeItem The lowest index that is to be skipped.
    ...
    /**
     * Returns TRUE if the Cron Specification is specified. FALSE otherwise. This is true if the specification has more than one entry
    ...
    /**
     * Returns TRUE if the Cron Specification is specified as a single value. FALSE otherwise. This is true only if there is only
    ...
    /**
     * Converts a range into natural language
     *
     * @param array $spec
     * @param type $entryFunction
     * @param type $p1
     * @return string
     */
    ...
    /**
     * Converts an entry from the minute specification to natural language.
     *
     * @param type $elem
     * @return string
     */
    ...
    /**
     * Converts an entry from the hour specification to natural language.
     *
     * @param type $elem
     * @param type $asBetween
     * @return string
     */
    ...
    /**
     * Converts an entry from the day of month specification to natural language.
     *
     * @param type $elem
     * @return string
     */
    ...
    /**
     * Converts an entry from the month specification to natural language.
     *
     * @param type $elem
     * @return string
     */
    ...
    /**
     * Converts an entry from the year specification to natural language.
     *
     * @param type $elem
     * @return string
     */
    ...
    /**
     * Returns the current cron specification in natural language.
     *
     * @return string A string containing a natural language text.
     */
    final public function asNaturalLanguage() {
        ...
        switch ($classDaysOfMonth . $classMonths) {
            ...
            case '00':
                $txtDate = ($switchForceDateExplaination ? ' ' . $this->natlangApply('elemDOM: on_every_day') : '');
                break;
